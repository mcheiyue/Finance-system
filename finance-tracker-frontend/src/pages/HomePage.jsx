...
import {
  ...
  DatePicker, theme, Typography, Avatar, App
} from 'antd';
import { 
    ...
    <div style={{ margin: '0 auto' }}>
      ...
      <Card variant="borderless" title={<Space><AppstoreOutlined /><span>账单明细</span></Space>} extra={<Space>{selectedRowKeys.length > 0 && (<Button danger icon={<DeleteOutlined />} onClick={handleBatchDelete}>删除</Button>)}<Button type="primary" icon={<PlusOutlined />} onClick={showModal}>记一笔</Button></Space>}>
        <Spin spinning={loading}>
          <Table rowSelection={{ selectedRowKeys, onChange: setSelectedRowKeys }} dataSource={displayData} columns={columns} rowKey="id" size="middle" pagination={{ current: currentPage, pageSize, total: displayData.length, onChange: (p, s) => { setCurrentPage(p); setPageSize(s); }, showTotal: t => `共 ${t} 条` }} />
        </Spin>
      </Card>
      ...
      <Modal title="新增记录" open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={500}>
        <Form form={form} layout="vertical" onFinish={handleFinish} initialValues={{ type: 'expense' }} style={{ marginTop: 20 }}>
          ...
          <Form.Item name="amount" label="金额" rules={[{ required: true }]}>
             {/* 3. 输入框深色模式优化：在 index.css 已全局处理，这里无需额外内联样式 */}
