...
const typeConfig = useMemo(() => ({
    ...
    innerRadius: 0.5,
    autoFit: true,
    scale: { color: { domain: ['收入', '支出'], range: [token.colorSuccess, token.colorError], }, },
    label: { text: (d) => { const percent = totalTypeAmount > 0 ? d.total / totalTypeAmount : 0; return `${d.type} ${(percent * 100).toFixed(0)}%`; }, position: 'outside', style: { fontSize: 12, fill: token.colorTextSecondary, fontWeight: 'bold', }, connector: true, },
    legend: { color: { position: 'right', layout: { justifyContent: 'flex-start' }, itemLabelFill: token.colorText, maxRows: 10, }, },
    style: { lineWidth: 2, stroke: token.colorBgContainer, },
    tooltip: { title: (d) => d.type, items: [ (d) => ({ name: '金额', value: `¥${Number(d.total).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}`, color: d.type === '收入' ? token.colorSuccess : token.colorError }), (d) => ({ name: '占比', value: `${(d.total / totalTypeAmount * 100).toFixed(1)}%`, }), ], },
  }), [typeData, token, totalTypeAmount]);
  ...
  const categoryConfig = useMemo(() => ({
    ...
    label: { text: (d) => { const percent = totalCategoryAmount > 0 ? d.total / totalCategoryAmount : 0; return `${d.category} ${(percent * 100).toFixed(0)}%`; }, position: 'outside', style: { fill: token.colorTextSecondary, }, connector: true, },
    legend: { color: { position: 'bottom', layout: 'horizontal', itemSpacing: 8, itemLabelFill: token.colorText, cols: 4, maxRows: 2, }, },
    style: { lineWidth: 2, stroke: token.colorBgContainer, },
    tooltip: { title: (d) => d.category, items: [ (d) => ({ name: '金额', value: `¥${Number(d.total).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}`, }), (d) => ({ name: '占比', value: `${(d.total / totalCategoryAmount * 100).toFixed(1)}%`, }), ], },
  }), [categoryData, chartColors, token, totalCategoryAmount]);
        ...
        <Row gutter={[48, 24]}>
          <Col xs={24} lg={12}>
            ...
            <div style={{ height: 350, width: '100%' }}>
              {totalTypeAmount > 0 ? (<Pie {...typeConfig} />) : (<Empty description="暂无收支数据" image={Empty.PRESENTED_IMAGE_SIMPLE} />)}
            </div>
          </Col>
          <Col xs={24} lg={12}>
    ...
    },
    {
      ...
      children: (
        <div style={{ height: 400, padding: '0 20px' }}>
          {trendData.length > 0 ? (<Area {...trendConfig} />) : (<div style={{ height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Empty description="暂无趋势数据" /></div>)}
        </div>
      )
    }
  ];
