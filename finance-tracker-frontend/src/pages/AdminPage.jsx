...
import { 
  Table, Card, Button, Tabs, Tag, message, Popconfirm, Spin, theme, Space 
} from 'antd';
import { 
...
export default function AdminPage() {
  ...
  const handleDeleteUser = async (id) => {
    try {
      ...
      message.error('删除失败: ' + (err.response?.data || err.message));
    }
  };
  useEffect(() => {
    loadUsers();
  ...
  const userColumns = [
    ...
    },
    {
      title: '操作', key: 'action',
      render: (_, record) => {
        if (record.username === 'admin') return <Tag>不可操作</Tag>;
        return (
          <Popconfirm
            title="高危操作警告"
          ...
          >
            <Button danger type="primary" size="small" icon={<DeleteOutlined />}>销号</Button>
          </Popconfirm>
        );
      }
    }
  ...
  const transactionColumns = [
    { 
      title: '用户ID', dataIndex: 'userId', width: 150, ellipsis: true,
      render: (text) => <Tag>{text}</Tag> 
    },
    { 
      title: '类型', dataIndex: 'type', width: 80,
      render: t => <span style={{color: t==='income'?token.colorSuccess:token.colorError}}>{t==='income'?'收入':'支出'}</span>
    },
    ...
    { 
      title: '日期', dataIndex: 'timestamp', width: 160,
  ...
  const tabItems = [
    ...
    {
      key: '2',
      label: <span><UnorderedListOutlined /> 全站审计</span>,
      children: <Table dataSource={transactions} columns={transactionColumns} rowKey="id" />
    }
  ];

  return (
    <Card 
      title={<Space><SafetyCertificateOutlined style={{color: token.colorPrimary}} /> 管理员控制台</Space>}
      variant={false}
    ...
    >
      <Spin spinning={loading}>
        <Tabs 
          defaultActiveKey="1" 
          items={tabItems} 
          onChange={(key) => { if(key === '2') loadAllTransactions(); }} 
        />
      </Spin>
    </Card>
  );
